...
void PlayerbotShamanAI::DropTotems()
{
    ...
    {
        if (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FROST && FROST_RESISTANCE_TOTEM > 0)
            m_ai->CastSpell(FROST_RESISTANCE_TOTEM);
        else if (spec == SHAMAN_SPEC_ELEMENTAL && TOTEM_OF_WRATH > 0)
            m_ai->CastSpell(TOTEM_OF_WRATH);
        // If the spec didn't take totem of wrath, use flametongue
        else if ((spec != SHAMAN_SPEC_ELEMENTAL || TOTEM_OF_WRATH == 0) && FLAMETONGUE_TOTEM > 0)
    ...
    {
        if (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_NATURE && NATURE_RESISTANCE_TOTEM > 0)
    ...
    {
        if (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_FIRE && FIRE_RESISTANCE_TOTEM > 0)
            m_ai->CastSpell(FIRE_RESISTANCE_TOTEM);
        else if (MANA_SPRING_TOTEM > 0)
...
void PlayerbotShamanAI::UseCooldowns()
{
    ...
    if (BLOODLUST > 0 && (!GetMaster()->HasAura(BLOODLUST, EFFECT_INDEX_0)))
        m_ai->CastSpell(BLOODLUST);
    else if (HEROISM > 0 && (!GetMaster()->HasAura(HEROISM, EFFECT_INDEX_0)))
