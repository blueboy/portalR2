        ...
        case CMSG_TOGGLE_PVP:
        {
            ...
            for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
            {
                ...
                if (bot->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP))
                {
                    ...
                    {
                        if (bg->isArena())
                            bot->GetPlayerbotAI()->SetScenarioType(PlayerbotAI::SCENARIO_PVP_ARENA);
                        else
                            bot->GetPlayerbotAI()->SetScenarioType(PlayerbotAI::SCENARIO_PVP_BG);
                    }
                    else if (bot->IsInDuel())
                    {
                        bot->GetPlayerbotAI()->SetScenarioType(PlayerbotAI::SCENARIO_PVP_DUEL);
                    }
                    else					
                        bot->GetPlayerbotAI()->SetScenarioType(PlayerbotAI::SCENARIO_PVP_OPENWORLD);
                }
                else
                    bot->GetPlayerbotAI()->SetScenarioType(PlayerbotAI::SCENARIO_PVE);
            }
            return;
        }
